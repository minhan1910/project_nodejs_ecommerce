...
export class CrudService<T> extends BaseService {
  ...
  async findOne(condition: any, projection: any = {}) {
    return await this.model.findOne(condition, projection);
  }
  ...
  async deleteManyById(ids: string[]) {
    const docs = await this.model.deleteMany({ _id: { $in: ids } });
    if (!docs) {
      throw new Error("Data not found");
    }
    return true;
  ...
  async deleteMany() {
    const docs = await this.model.deleteMany({});
    if (!docs) {
      throw new Error("Data not found");
    }
    return true;
